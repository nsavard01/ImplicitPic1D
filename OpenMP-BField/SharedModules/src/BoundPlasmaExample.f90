program BoundPlasmaExample
    ...
    real(real64), allocatable :: rho_i(:), E_test(:)
    ...
    allocate(rho_i(NumberXNodes), E_test(NumberXNodes+1))
    ...
    E_test = globalSolver%EField
    call globalSolver%evaluateEFieldFutureTime(globalWorld)
    print *, 'KE_f + PE_f - KE_i - PE_i:', KE_f + PE_f - KE_i - PE_i
    print *, ''

    print *, 'S values:'
    print *, currDel_t * 2.0d0 * SUM(globalSolver%J(1,:)) + eps_0 * (globalSolver%EField(1) - E_test(1))
    print *, currDel_t * SUM(globalSolver%J(2:NumberXNodes, :), DIM = 2) + eps_0 * (globalSolver%EField(2:NumberXNodes) - E_test(2:NumberXNodes))
    print *, currDel_t * 2.0d0 * SUM(globalSolver%J(NumberXNodes+1,:)) + eps_0 * (globalSolver%EField(NumberXNodes+1) - E_test(NumberXNodes+1))
