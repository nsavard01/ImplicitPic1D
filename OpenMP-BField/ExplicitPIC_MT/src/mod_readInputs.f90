...
contains
    ...
    subroutine readInitialInputs(InitFilename, simulationTime, n_ave, T_e, T_i, numDiagnosticSteps, fractionFreq, averagingTime, numThread, randGen)
        ...
        type(mt19937), allocatable, intent(out) :: randGen(:)
        character(len=*), intent(in) :: InitFilename
        integer(int32) :: io, i
        ...
        allocate(randGen(numThread))
        do i = 1, numThread
            call randGen(i)%initialize(123456*i*11 + i*7)
        end do
    ...
    subroutine readParticleInputs(filename, numberChargedParticles, randGen, T_e, T_i, numThread, world, particleList, targetParticleList)
        type(Particle), allocatable, intent(out) :: particleList(:)
        ...
        type(mt19937), intent(inout) :: randGen(numThread)
        ...
        if (numberChargedParticles > 0) then
            ...
            do j=1, numberChargedParticles
                ...
                call particleList(j) % generate3DMaxwellian(Ti(j), randGen)
                call particleList(j)% initializeRandUniform(randGen)
