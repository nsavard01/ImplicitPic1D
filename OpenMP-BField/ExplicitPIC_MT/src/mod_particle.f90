...
contains
    ...
    subroutine initializeRandUniform(self, randGen)
        class(Particle), intent(in out) :: self
        ...
        do i = 1, self%N_p(iThread)
            self%phaseSpace(1, i, iThread) = randGen(iThread)%genrand64_real1()* real(NumberXNodes-1) + 1.0d0
        end do
        !$OMP end parallel
    ...
    subroutine generate3DMaxwellian(self, T, randGen)
        ...
        iThread = omp_get_thread_num() + 1
        do i = 1, self%N_p(iThread)
            U1 = randGen(iThread)%genrand64_real1()
            U2 = randGen(iThread)%genrand64_real1()
            U3 = randGen(iThread)%genrand64_real1()
            U4 = randGen(iThread)%genrand64_real1()
