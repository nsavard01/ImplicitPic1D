...
inline void tri_maxwellian_3D(double& v_x, double& v_y, double& v_z, 
    const std::vector<double>& v_therm, const std::vector<double>& v_drift) {
    // full maxwellian with independent components
    double R_1 = pcg32_random_r();
    double R_2 = pcg32_random_r();
    double R_3 = pcg32_random_r();
    double R_4 = pcg32_random_r();
    double coeff = std::sqrt(-2.0 * std::log(R_1));
    v_x = v_drift[0] + v_therm[0] * coeff * std::cos(2.0 * M_PI * R_2); 
    v_y = v_drift[1] + v_therm[1] * coeff * std::sin(2.0 * M_PI * R_2);
    v_z = v_drift[2] + v_therm[2] * std::sqrt(-2.0 * std::log(R_3)) * std::cos(2.0 * M_PI * R_4);
}
