module mod_domain
    ...
    integer(int32) :: schemeNum = 0 ! represents NGP
...
contains
    ...
    type(Domain) function domain_constructor(GeomFilename) result(self)
        ! Construct domain object, initialize grid, dx_dl, and nodeVol.
        character(len=*), intent(in) :: GeomFilename
        real(real64) :: L_domain, debyeLength
        integer(int32) :: gridType, leftBoundary, rightBoundary, io
        print *, ""
        print *, "-------------------"
        ...
        call self%constructGrid(debyeLength, L_domain, gridType)
        if (schemeNum == 0) then
            print *, "Scheme is NGP"
        else
            print *, "Scheme is CIC"
        ...
        print *, "Grid Length is:", self%grid(NumberXNodes) - self%grid(1)
        print *, "Grid type is:", gridType
        print *, "Left boundary type:", self%boundaryConditions(1)
        print *, "Right boundary type:", self%boundaryConditions(NumberXNodes)
        print *, "------------------"
        print *, ""
    ...
    subroutine constructSineGrid(self, del_x, L_domain)
        ...
        self%nodeVol(1) = self%dx_dl(1)/2.0d0
        self%nodeVol(NumberXNodes) = self%dx_dl(NumberXNodes-1)/2.0d0
