module mod_particle
    ...
    ! For CIC, decide to have particle logical space start at 1.5, end at NumberXNodes - 0.5
    type :: Particle
    ...
    subroutine initialize_randUniform(self, world, irand)
        ...
        do i=1, NumberXNodes
            ...
            if (world%boundaryConditions(i) /= 0) then
                ...
                numInCell = INT(self%N_p * world%dx_dl(i)/L_domain/2.0d0)
                call getRandom(self%phaseSpace(1,idxLower:idxLower + numInCell-1), irand)
                numPerCell(i) = numInCell
                if (i == 1) then
                    self%phaseSpace(1, idxLower:idxLower + numInCell - 1) = self%phaseSpace(1, idxLower:idxLower + numInCell - 1) * 0.5d0 + 1.0d0
                else
                    self%phaseSpace(1, idxLower:idxLower + numInCell - 1) = NumberXNodes - self%phaseSpace(1, idxLower:idxLower + numInCell - 1) * 0.5d0
                end if
            else
                numInCell = INT(self%N_p * world%dx_dl(i)/L_domain)
                call getRandom(self%phaseSpace(1,idxLower:idxLower + numInCell-1), irand)
