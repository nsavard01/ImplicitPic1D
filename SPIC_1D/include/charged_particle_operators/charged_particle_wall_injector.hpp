...
public:
    ...
    charged_particle_wall_injector(std::vector<int>& particle_indx, std::vector<double>& current_density, std::vector<std::vector<double>>& v_3D, 
        std::vector<double>& v_therm, std::vector<double>& particle_location);
    void setup_diagnostics(const std::string& dir_name, const std::vector<charged_particle>& particle_list) override;
