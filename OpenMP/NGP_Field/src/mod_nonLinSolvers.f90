...
contains
    ...
    subroutine solveDivAmpereAnderson(solver, particleList, world, del_t)
        ...
        do i = 1, maxIter
            ...
            !call dgels('N', NumberXHalfNodes, m_k, 1, fitMat(:, 1:m_k), NumberXHalfNodes, alpha(1:ldb), ldb, work, lwork, info)
    ...
    subroutine solveJFNK(del_t)
        ...
        iterm = 0
        globalSolver%EField_f = globalSolver%EField
        xcurSolver = globalSolver%EField_f
        rpar(1) = del_t
        !dnrm2(NumberXHalfNodes, fcurSolver, 1)
        !print *, "initial norm is:", initialNorm
        !eps_r * SQRT(real(NumberXHalfNodes))
