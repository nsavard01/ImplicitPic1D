...
contains
    ...
    subroutine depositJ(solver, particleList, world, del_t)
        ...
        particleList(1)%workSpace(world%threadHalfNodeIndx(1, iThread):world%threadHalfNodeIndx(2, iThread), 1) = 0.5d0 * (solver%EField(world%threadHalfNodeIndx(1, iThread):world%threadHalfNodeIndx(2, iThread)) + &
            solver%EField_f(world%threadHalfNodeIndx(1, iThread):world%threadHalfNodeIndx(2, iThread)))
        !$OMP barrier
        ! Now apply smoothing if on
        if (world%gridSmoothBool) then
            call world%smoothField(particleList(1)%workSpace(1:NumberXHalfNodes,1), solver%EField_half, iThread)
            solver%EField_half(world%threadHalfNodeIndx(1, iThread):world%threadHalfNodeIndx(2, iThread)) = solver%EField_half(world%threadHalfNodeIndx(1, iThread):world%threadHalfNodeIndx(2, iThread))&
                /world%dx_dl(world%threadHalfNodeIndx(1, iThread):world%threadHalfNodeIndx(2, iThread))
        else
            solver%EField_half(world%threadHalfNodeIndx(1, iThread):world%threadHalfNodeIndx(2, iThread)) = particleList(1)%workSpace(world%threadHalfNodeIndx(1, iThread):world%threadHalfNodeIndx(2, iThread), 1)&
                /world%dx_dl(world%threadHalfNodeIndx(1, iThread):world%threadHalfNodeIndx(2, iThread))
        end if
        !$OMP barrier
