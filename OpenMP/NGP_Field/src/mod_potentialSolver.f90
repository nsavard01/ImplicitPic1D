module mod_potentialSolver
    ...
    type :: potentialSolver
        ...
        real(real64) :: rho_const, BFieldMag, BField(3), BFieldAngle, RF_rad_frequency, RF_half_amplitude, J_tot_ave, phi_left_f, phi_right_f
...
contains
    ...
    type(potentialSolver) function potentialSolver_constructor(world, leftVoltage, rightVoltage, BFieldMag, angle, RF_frequency) result(self)
        ...
        self%b_tri(NumberXNodes), self%c_tri(NumberXHalfNodes), self%EField(NumberXHalfNodes), self%EField_f(NumberXHalfNodes), self%EField_half(NumberXHalfNodes))
        ...
        self%phi_left_f = self%phi(1)
        self%phi_right_f = self%phi(NumberXNodes)
    ...
    function getError_tridiag_Poisson(self, world) result(res)
        ...
        do i = 1, NumberXNodes
            ...
            CASE(2)
                if (i == 1) then
                    res = res + (2.0d0 * eps_0 * (self%EField_f(1)/world%dx_dl(1))/(self%rho(1)) - 1.0d0)**2
                else
                    res = res + (-2.0d0 * eps_0 * (self%EField_f(NumberXHalfNodes)/world%dx_dl(NumberXHalfNodes))/(self%rho(i)) - 1.0d0)**2
                end if
            CASE(3)
                if (i == 1) then
                    res = res + (eps_0 * (self%EField_f(1)/world%dx_dl(1) - self%EField_f(NumberXHalfNodes)/world%dx_dl(NumberXHalfNodes))/(self%rho(1)) - 1.0d0)**2
                end if
            END SELECT
        end do
    ...
    subroutine solve_tridiag_Ampere(self, world, del_t)
        ...
        self%J_tot_ave = (SUM(self%J * world%dx_dl) + (eps_0/del_t) * (self%phi_right_f - self%phi_left_f - self%phi(NumberXNodes) + self%phi(1)))/world%L_domain
        self%EField_f = (self%J_tot_ave - self%J) * world%dx_dl * del_t / eps_0 + self%EField
    ...
    function getError_tridiag_Ampere(self, world, del_t) result(res)
        ...
        type(Domain), intent(in) :: world
        real(real64), intent(in) :: del_t
        real(real64) :: res(NumberXNodes)
        self%J_tot_ave = (SUM(self%J * world%dx_dl) + (eps_0/del_t) * (self%phi_right_f - self%phi_left_f - self%phi(NumberXNodes) + self%phi(1)))/world%L_domain
        res = eps_0 * (self%EField_f - self%EField)/world%dx_dl/del_t + self%J - self%J_tot_ave
    ...
    function getEnergyFromBoundary(self, world, del_t) result(res)
        ...
        real(real64), intent(in) :: del_t
        real(real64) :: res
        res = del_t * self%J_tot_ave * (self%phi_left_f + self%phi(1) - self%phi_right_f - self%phi(NumberXNodes)) * 0.5d0
    ...
    subroutine setRFVoltage(self, world, timeFuture)
        ...
        if (world%boundaryConditions(1) == 4) then
            self%phi_left_f = self%RF_half_amplitude * SIN(self%RF_rad_frequency * (timeFuture))
        else
            self%phi_right_f = self%RF_half_amplitude * SIN(self%RF_rad_frequency * (timeFuture))
        end if
    ...
    subroutine resetVoltage(self)
        ...
        self%phi(1) = self%phi_left_f
        self%phi(NumberXNodes) = self%phi_right_f
    ...
    subroutine aveRFVoltage(self, accumBool, phi_average, RF_ave, numSteps, world)
        ...
        if (accumBool) then
            phi_average = phi_average + self%phi 
            if (world%boundaryConditions(1) == 4) then
                RF_ave = RF_ave + self%phi_left_f
            else if (world%boundaryConditions(NumberXNodes) == 4) then
                RF_ave = RF_ave + self%phi_right_f
            end if
        else
            phi_average = phi_average/numSteps
            self%phi = phi_average
