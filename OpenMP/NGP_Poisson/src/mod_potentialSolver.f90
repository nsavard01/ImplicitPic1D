module mod_potentialSolver
    ...
    type :: potentialSolver
        real(real64), allocatable :: phi(:), J(:), rho(:), rho_f(:), phi_f(:), EField(:) !phi_f is final phi, will likely need to store two arrays for phi, can't be avoided
...
contains
    ...
    type(potentialSolver) function potentialSolver_constructor(world, leftVoltage, rightVoltage, RF_frequency, timeCurrent) result(self)
        ...
        allocate(self % J(NumberXHalfNodes), self%rho(NumberXNodes), self % phi(NumberXNodes), self%rho_f(NumberXNodes), self % phi_f(NumberXNodes), self%a_tri(NumberXHalfNodes), &
    ...
    subroutine makeConstSourceTerm(self, world)
        ...
        do i =1, NumberXNodes
            SELECT CASE (world%boundaryConditions(i))
            CASE(0)
                self%rho(i) = self%rho(i)/eps_0 - (self%phi(i) - self%phi(i-1))/world%dx_dl(i-1) + (self%phi(i+1) - self%phi(i))/world%dx_dl(i)
            CASE(1,3,4)
                self%rho(i) = self%phi_f(i)
            CASE(2)
                if (i == 1) then
                    self%rho(i) = self%rho(i)/eps_0 -2.0d0 * (self%phi(i) - self%phi(i+1))/world%dx_dl(i)
                else if (i == NumberXNodes) then
                    self%rho(i) = self%rho(i)/eps_0 -2.0d0 * (self%phi(i) - self%phi(i-1))/world%dx_dl(i-1)
                end if
            END SELECT
        end do
