...
contains
    ...
    subroutine depositJ(solver, particleList, world, del_t)
        ...
        solver%EField_half(1) = (solver%EField(NumberXHalfNodes) + solver%EField_f(NumberXHalfNodes) + &
            2.0d0 * solver%EField(1) + 2.0d0 * solver%EField_f(1) + solver%EField(2) + solver%EField_f(2))/8.0d0
        do j = 2, NumberXHalfNodes-1
            solver%EField_half(j) = (solver%EField(j-1) + solver%EField_f(j-1) + &
            2.0d0 * solver%EField(j) + 2.0d0 * solver%EField_f(j) + solver%EField(j+1) + solver%EField_f(j+1))/8.0d0
        end do
