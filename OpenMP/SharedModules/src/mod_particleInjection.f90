...
contains
    ...
    subroutine injectAtBoundary(particleList, T_e, T_i, irand, world)
        type(Particle), intent(in out) :: particleList(2)
        type(Domain), intent(in) :: world
        integer(int32), intent(in out) :: irand(numThread)
        real(real64), intent(in) :: T_e, T_i
        integer(int32) :: i,j, iThread, numInjected
        real(real64) :: ionSoundSpeed
        ! Reflux particles
        ...
        do i = 1, numInjected
            ...
            if (world%boundaryConditions(1) == 2) then
                ...
                particleList(1)%phaseSpace(1, particleList(1)%N_p(iThread) + i, iThread) = 1
                particleList(2)%phaseSpace(1, particleList(2)%N_p(iThread) + i, iThread) = 1
            else
                particleList(1)%phaseSpace(2, particleList(1)%N_p(iThread) + i, iThread) = -ABS(particleList(1)%phaseSpace(2, particleList(1)%N_p(iThread) + i, iThread))
                particleList(2)%phaseSpace(2, particleList(2)%N_p(iThread) + i, iThread) = -ABS(particleList(2)%phaseSpace(2, particleList(2)%N_p(iThread) + i, iThread))
                particleList(1)%phaseSpace(1, particleList(1)%N_p(iThread) + i, iThread) = NumberXNodes
                particleList(2)%phaseSpace(1, particleList(2)%N_p(iThread) + i, iThread) = NumberXNodes
            end if
        end do
