...
def compareRefToDatasRes(dataList, labelList, ref):
    ...
    for i in range(len(dataList)):
        ...
        ax1.plot(data.grid, 100 * (n_e-n_e_ref)/n_e_ref, linestyle = '--', marker = '.',label = labelList[i])
        ...
        ax2.plot(data.grid, 100 * (n_i-n_i_ref)/n_i_ref, linestyle = '--', marker = '.',label = labelList[i])
        ...
        ax3.plot(data.grid, 100 * (phi-phi_ref)/(phi_ref + 1e-15), linestyle = '--', marker = '.',label = labelList[i])
...
def compareModelToDatas(dataList, labelList, model):
    ...
    ax1.plot(model[0, :], model[2, :], linewidth = 2, label = 'Model')
    ax2.plot(model[0, :], model[3, :], linewidth = 2, label = 'Model')
    ...
    ax1.set_xlabel('Distance (m)', fontsize = 12)
    ax1.set_ylabel(r'$n_e$ (m$^{-3}$)', fontsize = 12)
    
    ax2.set_xlim(dataList[0].grid[0], dataList[0].grid[-1])
    ax2.set_xlabel('Distance (m)', fontsize = 12)
    ax2.set_ylabel(r'$n_+$ (m$^{-3}$)', fontsize = 12)
