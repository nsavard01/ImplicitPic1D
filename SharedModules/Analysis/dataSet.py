...
class dataSet:
    ...
    def __init__(self, filename = ""):
        ...
        for i in range(len(ParticleProperties)):
            self.particles[ParticleProperties.iloc[i]['name'][1:-1]] = {}
            self.particles[ParticleProperties.iloc[i]['name'][1:-1]]['mass'] = ParticleProperties.iloc[i]['mass']
            self.particles[ParticleProperties.iloc[i]['name'][1:-1]]['q'] = ParticleProperties.iloc[i]['q']
            self.particles[ParticleProperties.iloc[i]['name'][1:-1]]['w_p'] = ParticleProperties.iloc[i]['w_p']
