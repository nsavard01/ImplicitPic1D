...
contains
    ...
    subroutine artificialMaxwellianHeating(particleList, percent, T_e, T_i, irand)
        type(Particle), intent(in out) :: particleList(2)
        integer(int32), intent(in out) :: irand(numThread)
        real(real64), intent(in) :: percent, T_i, T_e
        real(real64) :: N_c, temp(2), work(4), R
        integer(int32) :: i, j, iThread, N_maxwellian, N_p, index, random
        temp = [T_e,T_i]        
        do j = 1, 1
            !$OMP parallel private(iThread, i, N_c, N_maxwellian)
            iThread = omp_get_thread_num() + 1
            N_p = particleList(j)%N_p(iThread)
            N_c = percent * N_p
            N_maxwellian = INT(N_c)
            random = irand(iThread)
            if (ran2(random) < N_c - N_maxwellian) then
                N_maxwellian = N_maxwellian + 1
            end if
            do i = 1, N_maxwellian
                R = ran2(random)
                index = INT(R * N_p) + 1
                work = particleList(j)%phaseSpace(:, N_p, iThread)
                call getMaxwellianSample(particleList(j)%phaseSpace(2:4, index, ithread),particleList(j)%mass, temp(j), random)
                particleList(j)%phaseSpace(:, N_p, iThread) = particleList(j)%phaseSpace(:, index, iThread)
                particleList(j)%phaseSpace(:, index, iThread) = work
                N_p = N_p - 1
            end do
