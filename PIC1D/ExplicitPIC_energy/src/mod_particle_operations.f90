...
contains
    ...
    subroutine move_particle_thread(part, EField, world, del_t, iThread)
        ...
        integer(int32) :: i, delIdx, refIdx, N_p, l_cell, direction
        real(real64) :: v_prime, q_over_m, l_i, x_i, x_f
        ...
        loopParticles: do i = 1, N_p
            l_i = part%phaseSpace(1, i, iThread)
            l_cell = int(l_i)
            ! First velocity change
            v_prime = part%phaseSpace(2, i, iThread) + q_over_m * EField(l_cell) * del_t 
            ! Get new position
            ...
            if (x_f > world%startX .and. x_f < world%endX) then 
                ...
                part%phaseSpace(1, i-delIdx, iThread) = real(l_cell, kind = 8) + (x_f - world%grid(l_cell))/world%dx_dl(l_cell)
    ...
    subroutine initialVRewind(solver, particleList, del_t, ionStepMult)
        ...
        loopSpecies: do j = 1, numberChargedParticles
            ...
            loopParticles: do i = 1, particleList(j)%N_p(iThread)
                particleList(j)%phaseSpace(2, i, iThread) = particleList(j)%phaseSpace(2, i, iThread) - 0.5d0 * (q_m_ratio) * solver%EField(int(particleList(j)%phaseSpace(1, i, iThread))) * del_t_temp
            end do loopParticles
            !$OMP end parallel
        end do loopSpecies
