...
contains
    ...
    subroutine loadParticleDensity(particleList, world, reset)
        ...
        integer(int32) :: i,j, cell, iThread
        real(real64) :: delD, partLoc
        !$OMP parallel private(iThread, j, i, cell, partLoc, delD)
                ...
                if (delD >= 0.5) then
                    if(cell == NumberXHalfNodes) then
                        if(world%boundaryConditions(NumberXNodes) == 3) then
                            particleList(i)%densities(1 , iThread)    = particleList(i)%densities(1, iThread)        + (delD - 0.5d0)
                            particleList(i)%densities(cell , iThread) = particleList(i)%densities(cell, iThread)     + (1.5d0 - delD)
                        else
                            particleList(i)%densities(cell , iThread) = particleList(i)%densities(cell, iThread)     + (1.5d0 - delD)
                        end if
                    else
                        particleList(i)%densities(cell + 1 , iThread) = particleList(i)%densities(cell + 1 ,iThread) + (delD - 0.5d0)
                        particleList(i)%densities(cell , iThread)     = particleList(i)%densities(cell, iThread)     + (1.5d0 - delD)
                    end if
                    ...
                    if(cell == 1) then
                        if(world%boundaryConditions(1) == 3) then
                            particleList(i)%densities(NumberXHalfNodes , iThread) = particleList(i)%densities(NumberXHalfNodes, iThread) + (0.5d0 - delD)
                            particleList(i)%densities(cell , iThread)             = particleList(i)%densities(cell, iThread)             + (0.5d0 + delD)
                        else
                            particleList(i)%densities(cell , iThread)             = particleList(i)%densities(cell, iThread)             + (0.5d0 + delD)
                        end if
                    else
                        particleList(i)%densities(cell - 1 , iThread)             = particleList(i)%densities(cell - 1, iThread)         + (0.5d0 - delD)
                        particleList(i)%densities(cell , iThread)                 = particleList(i)%densities(cell, iThread)             + (0.5d0 + delD)
                    end if
                end if
            end do        
    ...
    subroutine WriteParticleDensity(particleList, world, CurrentDiagStep, boolAverage, dirName)
        ...
        integer(int32), intent(in) :: CurrentDiagStep
        logical, intent(in) :: boolAverage
        real(real64) :: densities(NumberXHalfNodes)
    ...
    subroutine solveSimulation(solver, particleList, targetParticleList, nullCollisionList, world, del_t, irand, simulationTime)
        ...
        integer(int64) :: startTime, endTime, timingRate, collisionTime, potentialTime, moverTime, startTotal, endTotal, count
        ...
        do while(currentTime < simulationTime)
            ...
            if (currentTime < diagTime) then
                ...
                call solver%makeEField(world, currentTime)
                ! if(count == 1) then
                !     print*, "Rho:"
                !     print*, solver%rho
                !     print*, "Phi:"
                !     print*, solver%phi
                !     print*, "E:"
                !     print*, solver%EField, SIZE(solver%EField)
                !     stop
                ! end if
                call system_clock(endTime)
